...
const mockParticipants = [
  { name: 'You', avatarId: 'avatar-1', isHost: true, micOn: true, videoOn: true },
          ...
          <Card key={p.name} className="relative group overflow-hidden">
            <CardContent className="p-0">
              <div className="aspect-video bg-secondary rounded-lg flex items-center justify-center">
                ...
                {!p.videoOn && <div className="absolute inset-0 bg-card/70 backdrop-blur-sm"></div>}
              </div>
              ...
              <div className="absolute top-2 right-2 flex items-center gap-2">
                <div className={`p-1.5 rounded-full ${p.micOn ? 'bg-transparent' : 'bg-destructive'}`}>
                    {p.micOn ? <Mic className="h-4 w-4 text-white" /> : <MicOff className="h-4 w-4 text-destructive-foreground" />}
                </div>
                 <div className={`p-1.5 rounded-full ${p.videoOn ? 'bg-transparent' : 'bg-destructive'}`}>
              ...
              <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                {mockParticipants[0].isHost && !p.isHost && (
